...
class RequestBuilderTests: XCTestCase {
    ...
    func testBuilder_createRequestBuilder() {
        let parameters = RequestBuilderInitialParameters(urlScheme: RequestBuilderTests.testURLScheme,
    ...
    func testBuilder_createSpecificUserRequest() {
        ...
        let requestBuilder = RequestBuilder(using:parameters)
        XCTAssertNotNil(requestBuilder)
        let url = requestBuilder?.createURL(for: .persons,
                                            pagination: Pagination.defaultPagination,
                                            otherQueryItems: [URLQueryItem(name: "user_id", value: "1")])
