...
class RemoteDataFetcherTests: XCTestCase {
    ...
    static private let testURL = URL(string: "http://test.com")
    static private let testError = NSError(domain: "testDomain", code: 1, userInfo: nil)

    func testFetcher_notNilData() {
        let testNetworkProvider = TestNetworkProvider(data: RemoteDataFetcherTests.testData,
                                                          response: nil,
                                                          error: nil)
        let remoteDataFetcher = RemoteDataFetcher(using: DispatchQueue.global(qos: .userInitiated), networkProvider: testNetworkProvider)
    ...
    func testFetcher_nilDataNotNilError() {
        let testNetworkProvider = TestNetworkProvider(data: nil,
                                                           response: nil,
